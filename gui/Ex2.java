 ...
 *
 *
 * Assigment 2 is a set of classes and interfaces for basic geometric shapes. The main class for that is Ex2. which
 * controls the mouse clicks, movement and modes inside the GUI. Each geometric shape is defined in a private class all under the
 * interface of "GeoShape". GeoShapes defines our shapes by basic controls: like the function contains or area.
 * Later on, the shapes are further defined under GUIShape, both a class and an interface that defines each shape with
 * a color, wether It's filled or not and so. Inside the GUI, "contains" for each shape helps to target a specific shape,StdDraw defines
 * how drawing the shapes acts. Inside the GUI, we made a sorting method that can sort shapes by perimeter, area and so, that is defined under the class
 * ShapeComp. All the shapes that are visible on the screen are ordered in an ArrayList under the ShapeCollection class.
 * This class code gives us the ability to clear,add and remove shapes from the screen. Also, we can save drawn shapes to a txt file and then
 * load them back ! this is thanks to our ShapeCollection class, and the GUIShape constructor that builds GUIShapes from a string !
 * all of that is run from our given main, which simply launches the program.
 *
 *
 *
...
public class Ex2 implements Ex2_GUI {
    ...
    public void actionPerformed(String p) { //this method gets the string of what the user clicked and impliments it
        _mode = p;
        if (p.equals("Blue")) {
            _color = Color.BLUE;
            setColor(_color);
    ...
    public void mouseRightClicked(Point_2D p) { //method for right mouse click
        if (_mode.equals("Polygon") && _pp != null) { //if drawing a polygon, close it
